...
from datasets import PharyngitisFusionDataset
from models import ResNet18Fusion
from trainer import FusionTrainer, compute_metrics
...
def run_fusion_cv(
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_dummy, y_strat), 1):
        ...
        valid_df = df.iloc[va_idx].copy().reset_index(drop=True)

        scaler = StandardScaler()
        train_df[num_cols] = scaler.fit_transform(train_df[num_cols])
        valid_df[num_cols] = scaler.transform(valid_df[num_cols])

        train_ds = PharyngitisFusionDataset(train_df, tabular_cols=tabular_cols, transform=train_tfms)
        valid_ds = PharyngitisFusionDataset(valid_df, tabular_cols=tabular_cols, transform=valid_tfms)

        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=2)
        valid_loader = DataLoader(valid_ds, batch_size=batch_size, shuffle=False, num_workers=2)

        model = ResNet18Fusion(n_tabular=len(tabular_cols), pretrained=True)
        trainer = FusionTrainer(model, device=device, lr=lr)
    ...
    df_out = df.copy()
    df_out["oof_pred_fusion"] = oof_pred
    df_out.to_csv(output_csv, index=False)

    result_df.to_csv("fusion_5cv_results.csv", index=False)
